...
**Cierre (2026-08-11):** despliegue gradual (DEPL-01) implementado de punta a punta. **Backend puro** (`functions/logic.js`): `userFlagBucket(uid)` (hash determinista `% 100`, bucket estable 0-99 para que el usuario no "parpadee" entre activaciones) y `resolveUserFeatureFlags(source, uid)` — resuelve las flags efectivas por uid: flag global `false` = rollback/kill-switch para TODOS (incluso pilotos allowlist), `rollout: { <flag>: 0-100 }` = porcentaje por bucket (`pct 100` = todos), `allowlist: { <flag>: [uid,...] }` = pilotos siempre ON mientras la flag global sea `true`. **Backend callable**: `getFeatureFlags(uid)` en `index.js` cachea el `config/feature-flags` crudo (5 min, decisión D1), admite override de entorno `FLAG_{flag}` y resuelve por uid en cada llamada; los **12 call sites** (generatePlanning, recommendMethodologies, generateActivityVariants, gamificación, prompts externos, syncPlanningContext…) pasan `userId`. **Frontend**: espejo en `core.js` (`CLIENT_FEATURE_FLAGS`, `userFlagBucket`, `resolveUserFeatureFlags`, `loadFeatureFlags` con caché 5 min) — el `Layout` oculta "Gamificaciones" y "Prompts externos" según las flags efectivas del usuario (default apagadas = rollback visual sin romper rutas); `wizard.js` resuelve sus flags por `uid` con los helpers compartidos. **Seed idempotente** `scripts/seed-feature-flags.mjs` (flags apagadas por defecto, `--rollout <flag>=25`, `--allow <flag>=uid1,uid2`). 6 tests nuevos en `index.test.js` (determinismo/estabilidad del bucket, rollback con flag global off, allowlist override, rollout 0/100). **Unit 197/197**, E2E 13/13. El admin (custom claim `admin == true`) ve SIEMPRE todo activado por bypass (U17b: `resolveUserFeatureFlags(..., isAdmin)`), y el panel "Funcionalidades" de `institucional.js` (solo-admin) administra qué ven el resto: switch por flag + `% de docentes` (rollout) por flag. **Seed ejecutado en producción (2026-08-12):** `config/feature-flags` sembrado con `scripts/seed-feature-flags.mjs` (default flags off) — el doc queda listo para que el panel lo administre; nadie salvo el admin ve las funcionalidades hasta que el panel las abra. Pendiente: DEPL-02.

**Cierre U17b (2026-08-11):** panel de control de flags para admin. `resolveUserFeatureFlags(source, uid, isAdmin)` — el admin (custom claim `admin == true`) **SIEMPRE ve todo activado**, incluso con la flag global apagada (puede probar/verificar antes de abrir a otros); el panel controla qué ve el resto. Nueva onCall `updateFeatureFlags` (admin-only, validación `normalizeFlagUpdate` en `logic.js`: claves conocidas, rollout entero 0-100, allowlist de uids; invalida la caché en memoria y deja auditoría `update-feature-flags`). Frontend: card "Funcionalidades" en `institucional.js` (solo admin) con switch ON/OFF por flag + slider de porcentaje de docentes + Guardar/Recargar; `setFeatureFlagsFn`/`clearFeatureFlagsCache` en `core.js`. El `Layout` ahora lee las flags solo cuando hay sesión iniciada (watch sobre `store.user`), para que las rutas públicas no abran una conexión Firestore y el E2E `networkidle` se mantenga estable (regresión detectada tras el deploy U17 y corregida). 5 tests nuevos en `index.test.js` (bypass admin con flag global apagada, bypass con rollout 0, validador). **Unit 202/202**, E2E 13/13 (verificado tras deploy).
