...
- **Deploy happens on push to `main`** via `.github/workflows/deploy.yml` (hosting + functions, `--only hosting,functions`). It only triggers on `functions/**`, `public/**`, firebase config, and rules files — **docs-only commits skip deploy**. CI (`ci.yml`: unit tests + audit + E2E) runs on every push/PR.
